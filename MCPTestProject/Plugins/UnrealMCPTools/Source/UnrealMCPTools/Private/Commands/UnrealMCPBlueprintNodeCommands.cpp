...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintNodeCommands::HandleAddBlueprintFunctionCall(const TSharedPtr<FJsonObject>& Params)
{
    ...
    if (!Target.IsEmpty())
    {
        ...
        if (TargetClass)
        {
            ...
            // If FindFunctionByName failed on the class hierarchy, try creating the node
            // directly via SetExternalMember. This handles static library functions where
            // the UFunction may not be discoverable through normal iteration.
            if (!Function && !FunctionNode)
            {
                UE_LOG(LogTemp, Display, TEXT("Function not found via iteration, trying SetExternalMember for '%s' on '%s'"),
                       *FunctionName, *TargetClass->GetName());
                ...
                if (FunctionNode)
                {
                    ...
                    // Verify the node resolved properly by checking it has pins beyond just the exec pins
                    if (FunctionNode->Pins.Num() <= 0)
                    {
                        UE_LOG(LogTemp, Warning, TEXT("SetExternalMember produced a node with no pins for '%s::%s'"),
                    ...
                    {
                        UE_LOG(LogTemp, Display, TEXT("Created function node via SetExternalMember for '%s::%s' with %d pins"),
                               *TargetClass->GetName(), *FunctionName, FunctionNode->Pins.Num());
                    }
                }
            }
        }
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintNodeCommands::HandleAddBlueprintVariableGetNode(const TSharedPtr<FJsonObject>& Params)
{
    ...
    // Verify the variable exists on the blueprint
    ...
    if (!bVariableFound)
    {
        return FUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Variable '%s' not found on blueprint '%s'"), *VariableName, *BlueprintName));
    }
    ...
    // Set up the variable reference as a self member (blueprint's own variable)
    ...
    TSharedPtr<FJsonObject> ResultObj = MakeShared<FJsonObject>();
    ResultObj->SetStringField(TEXT("node_id"), VarGetNode->NodeGuid.ToString());
    ResultObj->SetStringField(TEXT("variable_name"), VariableName);
    return ResultObj;
}