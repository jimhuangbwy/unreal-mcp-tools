...
def register_blueprint_node_tools(mcp: FastMCP):
    ...
    def add_blueprint_enhanced_input_action_node(
        ...
        Requires a UInputAction asset created via create_input_action.
        Output pins: Triggered, Started, Ongoing, Canceled, Completed.

        Args:
            blueprint_name: Name of the target Blueprint
            action_name: Name of the InputAction asset (e.g., IA_Jump)
            node_position: Optional [X, Y] position in the graph
        ...
        try:
            ...
            logger.info(f"Adding enhanced input action node for '{action_name}' to blueprint '{blueprint_name}'")
            response = unreal.send_command("add_blueprint_enhanced_input_action_node", params)
    ...
    @mcp.tool()
    def add_blueprint_get_subsystem_node(
        ctx: Context,
        blueprint_name: str,
        subsystem_class: str,
        node_position = None
        ...
        """
        Add a Get Subsystem node (K2Node_GetSubsystemFromPC) to a Blueprint's event graph.
        Takes a PlayerController input and returns the specified subsystem with the correct type.

        Args:
            blueprint_name: Name of the target Blueprint
            subsystem_class: Subsystem class name (e.g., EnhancedInputLocalPlayerSubsystem)
            node_position: Optional [X, Y] position in the graph
        ...
        try:
            ...
            logger.info(f"Adding get subsystem node for '{subsystem_class}' to blueprint '{blueprint_name}'")
            response = unreal.send_command("add_blueprint_get_subsystem_node", params)

            if not response:
                logger.error("No response from Unreal Engine")
                return {"success": False, "message": "No response from Unreal Engine"}

            logger.info(f"Get subsystem node creation response: {response}")
            return response

        except Exception as e:
            error_msg = f"Error adding get subsystem node: {e}"
            logger.error(error_msg)
            return {"success": False, "message": error_msg}
    ...
    def add_blueprint_function_node(
        ...
        Args:
            blueprint_name: Name of the target Blueprint (e.g. "MyBP")
            target: Target for the function call. Can be:
                - A component name on the blueprint (e.g. "StaticMeshComponent0")
                - "self" for functions on the blueprint itself
                - A static library class name for global UE functions:
                  "KismetMathLibrary" (Multiply_FloatFloat, MakeRotator, Sin, Cos, etc.)
                  "KismetSystemLibrary" (PrintString, Delay, etc.)
                  "GameplayStatics" (GetActorOfClass, SpawnSound2D, etc.)
                  "KismetStringLibrary" (Concat_StrStr, etc.)
                  The "U" prefix is optional (both "KismetMathLibrary" and "UKismetMathLibrary" work).
            function_name: Name of the function to call (e.g. "Multiply_FloatFloat", "MakeRotator",
                          "AddActorWorldRotation", "K2_GetActorRotation")
    ...
    def add_blueprint_variable_get_node(
        ...
        This creates a "Get <VariableName>" node that outputs the variable's current value.
        The variable must already exist on the blueprint (use add_blueprint_variable first).

        Args:
            blueprint_name: Name of the target Blueprint (e.g. "RotatingCube")
            variable_name: Name of the variable to get (e.g. "RotationSpeed", "Health")
            node_position: Optional [X, Y] position in the graph
